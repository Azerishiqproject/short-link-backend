...
router.post("/register", async (req, res) => {
    ...
    await EmailVerification_1.EmailVerification.deleteMany({ email });
    const code = generateVerificationCode();
    const expiresAt = new Date(Date.now() + 15 * 60 * 1000); // 15 dakika
    const pending = await EmailVerification_1.EmailVerification.create({
        email,
        passwordHash,
        name,
  ...
  `;
    try {
        await (0, mailer_1.sendMail)({ to: email, subject: "E-posta Doğrulama Kodu", html });
    }
    catch (e) {
        // E-posta gönderilemezse pending kaydı temizleyip hata döndür
        await EmailVerification_1.EmailVerification.deleteOne({ _id: pending._id });
...
router.post("/refresh", async (req, res) => {
    try {
        ...
        const user = await User_1.User.findById(decoded.sub).select('role');
        if (!user)
...
const updateUserSchema = zod_1.z.object({
    ...
    referralCode: zod_1.z.string().length(6).regex(/^[A-Z0-9]{6}$/).optional(),
});
router.patch("/admin/users/:userId", auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        if (parsed.data.referralCode !== undefined) {
            ...
            const existsCode = await User_1.User.findOne({ referralCode: newCode, _id: { $ne: userId } }).select('_id');
            if (existsCode)
...
const verifyEmailSchema = zod_1.z.object({ verificationId: zod_1.z.string().min(1), code: zod_1.z.string().length(6) });
router.post("/verify-email", async (req, res) => {
    const parsed = verifyEmailSchema.safeParse(req.body);
    if (!parsed.success)
        return res.status(400).json({ error: parsed.error.flatten() });
    const { verificationId, code } = parsed.data;
    const pending = await EmailVerification_1.EmailVerification.findById(verificationId);
    if (!pending)
        return res.status(400).json({ error: "Doğrulama oturumu bulunamadı" });
    ...
    if (pending.code !== code) {
        ...
        await EmailVerification_1.EmailVerification.updateOne({ _id: pending._id }, { $set: { attempts } });
        return res.status(400).json({ error: "Kod hatalı" });
    }
    // Re-check email uniqueness right before creation
    const existing = await User_1.User.findOne({ email: pending.email });
    if (existing) {
        await EmailVerification_1.EmailVerification.deleteOne({ _id: pending._id });
        return res.status(409).json({ error: "Email already in use" });
    }
    ...
    if (pending.referrerId) {
        await User_1.User.findByIdAndUpdate(pending.referrerId, { $inc: { referralCount: 1 } });
        referralService_1.referralService.processRegistrationReferral(String(user._id)).catch(() => { });
    }
    await EmailVerification_1.EmailVerification.deleteOne({ _id: pending._id });
    const token = signAccess({ sub: String(user._id), role: user.role });
    const refresh = signRefresh({ sub: String(user._id) });
...
const resendSchema = zod_1.z.object({ verificationId: zod_1.z.string().min(1) });
router.post("/resend-code", async (req, res) => {
    ...
    await EmailVerification_1.EmailVerification.updateOne({ _id: verificationId }, { $set: { code, expiresAt, attempts: 0 } });
  ...
  `;
    try {
        await (0, mailer_1.sendMail)({ to: pending.email, subject: "Doğrulama Kodu", html });
    }
    catch (e) {
...
const updateUserRoleSchema = zod_1.z.object({
    ...
    role: zod_1.z.enum(["user", "admin", "advertiser"])
});
router.put("/admin/users/:userId/role", auth_1.requireAdmin, async (req, res) => {
    try {
        const { userId } = req.params;
        ...
        const user = await User_1.User.findByIdAndUpdate(userId, { role }, { new: true }).select("email name role createdAt");
        if (!user) {
            return res.status(404).json({ error: "User not found" });
        }
        return res.json({
