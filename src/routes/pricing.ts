...
router.delete("/:audience/:country", requireAdmin, async (req, res, next) => {
  try {
    const { audience, country } = req.params;
    const doc = await Pricing.findOne();
    if (!doc) return res.status(404).json({ error: "Pricing not found" });
    
    // Filter out the entry to delete
    const filteredEntries = doc.entries.filter(
      (entry: any) => !(entry.audience === audience && entry.country.toUpperCase() === country.toUpperCase())
    );
...
router.patch("/:audience/:country", requireAdmin, async (req, res, next) => {
  try {
    const { audience, country } = req.params;
    ...
    const entryIndex = doc.entries.findIndex(
      (entry: any) => entry.audience === audience && entry.country.toUpperCase() === country.toUpperCase()
    );
    
    if (entryIndex === -1) return res.status(404).json({ error: "Pricing entry not found" });
